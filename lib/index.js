var generateQueryString = require('./utils').generateQueryString,
    generateQueryStringItemLookup = require('./utils').generateQueryStringItemLookup,
...
var itemLookup = function(credentials) {
  ...
  return function(query, cb) {
    var url = generateQueryStringItemLookup(query, credentials),
      results;
    ...
    var promise = new Promise(function(resolve, reject) {
      ...
      request(url, function(err, response, body) {
        ...
        if (typeof response === 'undefined') {
          reject('Amazon response undefined');
        }
        ...
        if (response.statusCode !== 200) {
          ...
          parseXML(body, function(err, resp){
            results = resp.ItemLookupResponse.Items[0].Item[0];
            resolve(results);
          });
        }
      });
